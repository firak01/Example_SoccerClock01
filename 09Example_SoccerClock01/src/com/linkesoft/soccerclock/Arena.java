...
/**
 * 
 * Hauptklasse für die Animation der Fussballuhr, enthält Spieler, Ball, Toranzeige
 * 
 * @author Andreas Linke
 *
 */
public class Arena extends FrameLayout implements TimeAnimator.TimeListener {
	private final float interval_ms=3000; // wie lange dauert ein Schuss quer übers Feld
	public final TimeAnimator timeAnimator; // animator für regelmäßige Updates aller Views
	
	private final Player leftPlayer; // spielt für die Stunden
	private final Player rightPlayer; // spielt für die Minuten 
	...
	// oder bei Änderung der Stunden bzw. Minuten sich verfehlen
	private void calculateVelocities(float width, float height) {
		// zufällige vertikale Ball-Position
		float targetBallY = (float) (Math.random() * (height - ball.getHeight()));
		ball.vy = (targetBallY-ball.getY()) / interval_ms;
		// wohin muss der Spieler, damit er den Ball genau am Fuß trifft (beide Spieler sind gleich hoch)
		...
		if (ball.getX() < width / 2) {
			...
			if (TimeDisplay.currentHours() != leftPlayer.goals)
				targetPlayerY=targetBallY>getHeight()/2 ? 0 : height-rightPlayer.getHeight(); // läuft in die falsche Ecke, verfehlt den Ball
 ...
 */
	...
	public void onTimeUpdate(TimeAnimator animation, long totalTime_ms,
		...
		// Spieler ändern automatisch Richtung am oberen/unteren Rand
